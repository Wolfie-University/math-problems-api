...
class OptimizationGenerator extends BaseGenerator {
  generate() {
    // System scenariuszy
    // template: Teraz jest funkcją przyjmującą wszystkie zmienne dynamiczne
    const scenarios = [
      {
        id: "electronics",
      ...
      },
      {
        ...
        unit: "szt.",
        type: "density",
        ranges: { price: [30, 60], sales: [40, 80], stepSales: [1, 2] },
        template: (fruits, trees, stepFruits, stepTrees, subject) =>
          ...
          `Każde dodatkowe posadzone drzewo (powyżej liczby ${trees}) powoduje zmniejszenie plonu z każdego drzewa o ${stepFruits} kg (z powodu mniejszego nasłonecznienia). ` +
    ...
    if (scenario.type === "density") {
      return this.generateDensityProblem(scenario);
    } else {
      return this.generateRevenueProblem(scenario);
    }
  }

  // --- LOGIKA 1: PRZYCHÓD (CENA * ILOŚĆ) ---
  generateRevenueProblem(scenario) {
    const stepPrice = 1;
    ...
    do {
      ...
      // a = -stepSales
      // b = startPrice * stepSales - startSales
    ...
    // POPRAWKA: Wywołujemy funkcję template zamiast robić .replace()
    ...
    return this.createResponse({
      ...
      steps: [
        `Oznaczmy przez $$x$$ kwotę obniżki (lub liczbę kroków zmiany ceny).`,
        `Nowa cena: $$${startPrice} - x$$. Nowa sprzedaż: $$${startSales} + ${stepSales}x$$.`,
        `Funkcja przychodu $$P(x)$$ to iloczyn ceny i liczby sprzedanych sztuk:`,
        `$$P(x) = (${startPrice} - x)(${startSales} + ${stepSales}x)$$`,
        `Jest to funkcja kwadratowa o ramionach skierowanych w dół ($$a < 0$$), więc osiąga maksimum w wierzchołku.`,
        `Miejsca zerowe tej funkcji to momenty, gdy cena=0 ($$x=${startPrice}$$) lub sprzedaż=0.`,
        `Wierzchołek $$p$$ leży dokładnie pośrodku między miejscami zerowymi, lub liczymy go ze wzoru $$p = \\frac{-b}{2a}$$.`,
        `Po obliczeniach: $$p = ${x}$$ (optymalna obniżka).`,
        `Pytanie dotyczyło ${scenario.id === "hotel" || scenario.id === "electronics" ? "nowej ceny" : "kwoty obniżki"}.`,
        scenario.id === "hotel" || scenario.id === "electronics"
          ? `Optymalna cena: $$${startPrice} - ${x} = ${newPrice}$$ ${scenario.unit}.`
          : `Należy obniżyć cenę o $$${x}$$ ${scenario.unit}.`,
      ],
    });
  }

  // --- LOGIKA 2: SADOWNIK (ZAGĘSZCZENIE) ---
  generateDensityProblem(scenario) {
    const startFruits = MathUtils.randomInt(40, 60);
    ...
    // POPRAWKA: Przekazujemy subject (np. "drzew") do funkcji template
    const questionText = scenario.template(
      calculatedStartFruits,
      startTrees,
      lossPerTree,
      null, // stepTrees nie jest używane w tym template, dajemy null
      scenario.subject,
    );

    return this.createResponse({
      question: questionText,
      latex: `Plon(x) = (${calculatedStartFruits} - ${lossPerTree}x)(${startTrees} + x)`,
      image: this.generateParabolaSVG(
        calculatedStartFruits,
        startTrees,
        lossPerTree,
      ...
      steps: [
        `Niech $$x$$ oznacza liczbę dosadzonych drzew.`,
        `Liczba drzew po dosadzeniu: $$${startTrees} + x$$.`,
        `Plon z jednego drzewa: $$${calculatedStartFruits} - ${lossPerTree}x$$.`,
        `Funkcja łącznego plonu $$P(x)$$ to iloczyn liczby drzew i plonu z jednego drzewa:`,
        `$$P(x) = (${startTrees} + x)(${calculatedStartFruits} - ${lossPerTree}x)$$`,
        `Szukamy wierzchołka paraboli (maksimum).`,
        `Po wymnożeniu i obliczeniu pochodnej lub wzoru na wierzchołek: $$x_{opt} = ${targetX}$$.`,
  ...
  generateParabolaSVG(startP, startS, step, optX, maxRev, isDensity = false) {
    ...
    for (let x = -10; x <= 10; x += 0.5) {
      const relX = x;
      const relY = -(x * x) + 9; // Zwykła odwrócona parabola y = -x^2 + 9 jako wizualizacja
    ...
    const dashedLines = `
            <line x1="${center}" y1="${center - 90}" x2="${center}" y2="${center + 90}" stroke="#aaa" stroke-dasharray="4" />
            <text x="${center - 5}" y="${center + 110}" font-size="12" font-weight="bold">${optX}</text>
        `;

    return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg" style="border:1px solid #ddd; background:#fff">
            ${axis}
            <path d="${pathData}" stroke="#007bff" stroke-width="3" fill="none" transform="translate(0, 90)"/>
            <g transform="translate(0, 90)">${vertex}${dashedLines}</g>
        </svg>`;
  }
}
